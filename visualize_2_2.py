...
with open("outputs/per_8694617.txt") as f:
    ...
    for line in lines:
        ...
        if line.split()[4] == "matmult_mnk":
            y_mnk.append(float(line.split()[1]))
            x_mnk.append(float(line.split()[0]))
        elif line.split()[4] == "matmult_mkn":
            y_mkn.append(float(line.split()[1]))
            x_mkn.append(float(line.split()[0]))
        elif line.split()[4] == "matmult_nmk":
            y_nmk.append(float(line.split()[1]))
            x_nmk.append(float(line.split()[0]))
        elif line.split()[4] == "matmult_nkm":
            y_nkm.append(float(line.split()[1]))
            x_nkm.append(float(line.split()[0]))
        elif line.split()[4] == "matmult_kmn":
            y_kmn.append(float(line.split()[1]))
            x_kmn.append(float(line.split()[0]))
        elif line.split()[4] == "matmult_knm":
            y_knm.append(float(line.split()[1]))
            x_knm.append(float(line.split()[0]))
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
ax1.set_title("mkn value - Performance : gcc")    
ax1.set_ylabel('Performance (Mflops/s)')
ax1.set_xlabel('m/n/k value (int)')
ax1.plot(x,y_mnk, '-x', c='b', label='mnk data')
ax1.plot(x,y_mkn, '-x', c='r', label='mkn data')
ax1.plot(x,y_nmk, '-x', c='g', label='nmk data')
ax1.plot(x,y_nkm, '-x', c='c', label='nkm data')
ax1.plot(x,y_kmn, '-x', c='k', label='kmn data')
ax1.plot(x,y_knm, '-x', c='y', label='knm data')
leg = ax1.legend()
plt.show()
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
ax1.set_title("Memory - Performance : gcc Ofast")    
ax1.set_xlabel('Memory Footprint (kbytes)')
ax1.set_ylabel('Performance (Mflops/s)')
ax1.plot(x_mnk,y_mnk, '-x', c='b', label='mnk data')
ax1.plot(x_mkn,y_mkn, '-x', c='r', label='mkn data')
...
fig = plt.figure()
ax1 = fig.add_subplot(111)
ax1.set_title("mkn value - Performance: gcc Ofast")    
ax1.set_ylabel('Performance (Mflops/s)')
ax1.set_xlabel('m/n/k value (int)')
ax1.plot(x,y_mnk, '-x', c='b', label='mnk data')
ax1.plot(x,y_mkn, '-x', c='r', label='mkn data')
